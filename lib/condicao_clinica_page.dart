...
class _CondicaoClinicaPageState extends State<CondicaoClinicaPage> {
  ...
  Future<void> _carregarDados() async {
    try {
      ...
      final idioma = ['PT', 'US', 'ES'].contains(locale) ? locale : 'PT';

      setState(() {
        dados = jsonMap[idioma];
      });
    } catch (e) {
      setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(_formatarTitulo(widget.arquivoJson.replaceAll('_', ' ')),
        ),
      ),
      body: dados == null
  ...
  String _formatarTitulo(String chave) {
    const titulosMapeados = {
      'definicao': 'Definição',
      'epidemiologia': 'Epidemiologia',
      'fisiopatologia': 'Fisiopatologia',
      'manifestações_clinicas': 'Manifestações Clínicas',
      'diagnostico': 'Diagnóstico',
      'indicações_para_suporte_ventilatório': 'Indicações para Suporte Ventilatório',
      'conduta_geral': 'Conduta Geral',
      'prognostico': 'Prognóstico',
      'notas_adicionais': 'Notas Adicionais',
      'abordagem_clinica': 'Abordagem Clínica',
      'exames_essenciais': 'Exames Essenciais',
      'abordagem_inicial': 'Abordagem Inicial',
      'hemodinamicas': 'Hemodinâmicas',
      'respiratorias': 'Respiratórias',
      'neurologicas_e_metabolicas': 'Neurológicas e Metabólicas',
    };

    return titulosMapeados[chave] ??
        chave
            .replaceAll('_', ' ')
            .split(' ')
